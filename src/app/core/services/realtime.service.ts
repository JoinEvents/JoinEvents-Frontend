 ...
 * (reconnecting after drops) and relays chat messages and notifications the moment the server
 * sends them; screens keep their periodic refresh as a fallback.
 */
...
export class RealtimeService {
  ...
  private async start(token: string): Promise<void> {
    ...
    connection.on('ReceiveMessage', (m: LiveMessage) => this.zone.run(() => this.messages$.next(normalizeMessage(m))));
    ...
    try {
      ...
      // Offline or the API is unreachable: screens fall back to their periodic refresh.
...
function normalizeMessage(m: any): LiveMessage {
  return {
